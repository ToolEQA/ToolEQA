...
def gen_react(data_path, system_prompt_path, planing_prompt_path, user_prompt_path, nonkey_gen_reactuser_prompt_path, output_path, wrong_path):
    system_prompt = get_prompt(system_prompt_path)

    
    tool_box_selected = [ObjectLocation2D(debug=True), ObjectCrop(debug=True), GoNextPointTool(debug=True), FinalAnswerTool(debug=True), VisualQATool(debug=True)]
...
if __name__=="__main__":
    ...
    data_path = "output/color_revise.jsonl"
    wrong_path = 'color_wrong.json'
    # task_id = int(sys.argv[1]) 
    # output_path = f"output/color_reproduct.jsonl"
    output_path = f"output/color_reproduct.json"

    gen_react(data_path, system_prompt_path, planing_prompt_path, user_prompt_path, nonkey_user_prompt_path, output_path, wrong_path)
