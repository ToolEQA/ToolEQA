...
# 配置路径
input_json_path = 'data/location-location_remove_region_id_and_dedup.json'  # 替换为你的输入路径
...
for entry_idx, entry in enumerate(data):
    ...
    for idx, obj in enumerate(related_objects):
        ...
        region_id = object_region_map.get(obj_id)
        if region_id is None:
            print(f"[跳过] object_id {obj_id} 无 region_id 映射")
            continue

        if str(region_id) not in answer_list: # 如果 这个物体信息不在答案里面的话，那么直接删掉
    
            removed_indices.append(idx)
        else:
            obj["region_id"] = region_id  # 加上 region_id 信息
            key = (name, region_id)
            if key not in seen:
                seen.add(key)
...
# 可选：保存处理后的 JSON
output_path = 'data/location-location_revise_end_2.json'
