...
for idx, item in enumerate(data):
    ...
    if len(answer_list)>=2 and len(set(object_name))>=2:
        # print(item)
        # print(set(object_name), len(objects), item["sample_id"])
        count_greater_than_3 = count_greater_than_3 + 1
        if len(objects) != 4 :
            print(len(objects), item["sample_id"])
    else:
        data_new.append(item)

    all_object = all_object + 1

# output_path = 'data/location-location_revise_final.json'
