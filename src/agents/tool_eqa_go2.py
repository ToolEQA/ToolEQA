...
class EQAReactAgent(ReactCodeAgent):
    ...
    def step(self):
        ...
        action_queue = self.toolbox._tools["GoNextPointTool"].eqa_modeling.action_queue
        if len(action_queue) > 0:
            prompt = self.prompt.copy()
            prompt[-1]["content"] += f"Previous action sequence:\n{action_queue}\n"
        else:
            prompt = self.prompt

        try:
            llm_output = self.llm_engine(prompt, stop_sequences=["<end_action>", "Observation:"], image_paths=self.image)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cfg", help="config", type=str, default="./config/react-eqa-real.yaml")
    # Is there a blanket on the bed?
    # Is there a person on the sofa?
    # Is the door on the upper side of the white cabinet open?
    parser.add_argument("--question", help="eqa question", type=str, default="Is the gate on the upper side of the white file cabinet open?")
    parser.add_argument("--output", help="output direction", type=str, default="./results/real_test")
    ...
    system_prompt = open("prompt/system_prompt_real.txt", "r").read()
